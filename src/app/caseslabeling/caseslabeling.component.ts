...
export class CaseslabelingComponent implements OnInit, OnDestroy {
  ...
  constructor(private titleService: Title, private reviewsService: ReviewsService, private labelService: LabelService) { }
  ...
  getNextUnreviewedReview() {
    this.reviewSubscription = this.reviewsService.getUserFirstUnreviewedReview().subscribe(
      (review) => {
        this.review = review;
        this.loading = false;
        this.error = false;
  ...
  saveAndGetNextCase() {
    ...
    this.reviewSubscription = this.reviewsService.reviewCase(this.review?.id, this.labelsControl?.value).subscribe(
      (review) => {
        ...
        Toast.fire({
          icon: 'success',
          title: 'Case has been reviewed successfully, you can update review in reviews section',
        });
         // Fetch next case
         this.getNextUnreviewedReview();
      },
